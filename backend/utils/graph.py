...
def _whatsappconfig() -> tuple[str, str]:
    ...
    return f"https://{FACEBOOK_GRAPH_HOST}/{FACEBOOK_GRAPH_VERSION}", WHATSAPP_TOKEN
...
def _adsconfig() -> tuple[str, str]:
    if not META_AD_ACCESS_TOKEN:
        raise RuntimeError("META_AD_ACCESS_TOKEN is not set")
    return f"https://{FACEBOOK_GRAPH_HOST}/{FACEBOOK_GRAPH_VERSION}", META_AD_ACCESS_TOKEN


def ads_account_id() -> str:
    if not META_AD_ACCOUNT_ID:
        raise RuntimeError("META_AD_ACCOUNT_ID is not set")
    return META_AD_ACCOUNT_ID


async def ads_get(path: str, **params: Any) -> Any:
    base, token = _adsconfig()
    q = _clean(params)
    q["access_token"] = token
    label = f"ADS GET {path}"
...
async def ads_post(path: str, **fields: Any) -> Any:
    base, token = _adsconfig()
    label = f"ADS POST {path}"
...
async def ads_delete(path: str, **params: Any) -> Any:
    base, token = _adsconfig()
    q = _clean(params)
    q["access_token"] = token
    label = f"ADS DELETE {path}"
    logger.debug("API call %s | params=%s", label, {k for k in q if k != "access_token"})
